...
```java
// 过滤
Stream filteredStream = list.stream().filter(
  s -> s.startsWith("a")
);

// 映射
Stream mappedStream = list.stream().map(String::length);

// 排序
Stream sortedStream = list.stream().sorted();

// 去重
Stream distinctStream = list.stream().distinct();

// 截断
Stream limitedStream = list.stream().limit(2);

// 跳过
Stream skippedStream = list.stream().skip(1);
```
...
```java
// 聚合操作
Optional anyElement = list.stream().findAny();
Optional firstElement = list.stream().findFirst();
long count = list.stream().count();
Optional maxElement = list.stream()
    .max(Comparator.naturalOrder());
Optional minElement = list.stream()
...
// 归约
Optional reducedString = list.stream()
  .reduce((s1, s2) -> s1 + s2);
...
```java
Consumer test = System.out::println;
```
...
```java
Comparator comparator = Math::max;

int result = comparator.compare(1, 2);
// 返回 -1 

BinaryOperator maxOperator = Math::max;
...
```java
String str = "HELLO";
Supplier lowerCaseSupplier = str::toLowerCase;
...
Supplier lowerCaseSupplier = someStr::toLowerCase;
String lowerCase = lowerCaseSupplier.get();
// 返回 "hello"
```
...
Supplier methodSupplier = someObject::staticMethod;
