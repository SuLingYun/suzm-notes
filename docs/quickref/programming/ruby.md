...
| `do`           | 开始一个代码块并执行块中的代码，以  **end关键字**结束
...
`$-d`  | `$DEBUG` 的别名
`$-F`  | `$;` 的别名
`$-i`  | 在就地编辑模式下，此变量保存扩展，否则为零  可以指定启用（或禁用）就地编辑模式
`$-I`  | `$:` 的别名
...
类型    | 示例 | Class | 文档
:-- | -- | -- | --
`Integer` | a = 17                       | a.class > Integer   a.class.superclass > Numeric | [#][2]
`Float`   | a = 87.23                    | a.class > Float   a.class.superclass > Numeric   | [#][3]
`String`  | a = "Hello universe"         | a.class > String                                   | [#][4]
`Array`   | a = [12, 34]                 | a.class > Array                                    | [#][5]
`Hash`    | a = {type: "tea", count: 10} | a.class > Hash                                     | [#][6]
`Boolean` | a = false  a = true        | a.class > FalseClass   a.class > TrueClass       | [TrueClass][7] [FalseClass][8]
...
prepend                          | `a = "world"   a.prepend("hello ")`   `"hello world"`                                             | 在另一个字符串之前添加字符串
insert                           | `a = "hello"   a.insert(a.length, " world")`   `"hello world"`                                    | 在特定位置插入字符串
...
split                            | `string = "a b c d"   string.split`   `["a", "b", "c", "d"]`                                      | 将字符串转换为字符数组
join                             | `arr = ['a', 'b', 'c']   arr.join`  => `"abc"`                                                      | 将数组转换为字符串
to_i                             | `a = "49"   a.to_i` => `49`                               | 将字符串转换为整数
chop                             | `"abcd?".chop("?")` => `"abcd"`                              | 从字符串中删除最后一个字符
count                            | `str = "aaab"   str.count("a")`   `3`                  | 计算字符串中的字符
to_f                             | `a = "49"`   `a.to_f`   `49.0`                         | 将字符串转换为浮点数
to_sym                           | `a = "key"`   `a.to_sym`   `:key`                      | 将字符串转换为符号
match                            | `"abcd?".match(/ab/)` => `#`                 | 将模式转换为正则表达式并在字符串上调用其匹配方法
