...
auto p1 = std::make_unique(42);

// 创建方式2：直接构造
std::unique_ptr p2(new int(42));
...
尝试上锁，可以通过`std::unique_lock::owns_lock()`查看状态
...
- `std::launch::deferred`：入口函数的运行会被推迟到`std::future::get()`或者`std::future::wait()`被调用时。此时调用线程会直接运行线程入口函数，换言之，**不会创建子线程**
...
int main() {
  // 启动异步任务
  std::future fut = std::async(std::launch::async, taskFunction);
