...
`--collectCoverageFrom=` | 相对于 `rootDir` 的 `glob` 模式匹配需要从中收集覆盖信息的文件
`--colors` | 即便 `stdout` 不是 `TTY` 模式，也要强制高亮显示测试结果
`--config=` | 指定配置文件的路径
`--coverage[=]`, `--collectCoverage` | 将测试覆盖率信息输出为报告
`--coverageProvider=` | 指示应该使用哪个提供程序来检测代码的覆盖率
...
`--filter=` | 导出过滤功能的模块的路径
`--findRelatedTests ` | 查找并运行涵盖作为参数传入的以空格分隔的源文件列表的测试
`--forceExit` | 强制 `Jest` 在所有测试运行完后退出
`--help` | 显示帮助信息，类似于本页文档
`--ignoreProjects  ... ` | 忽略特定的测试项目
`--init` | 生成一个基础配置文件
...
`--maxConcurrency=` | 防止 `Jest` 同时执行超过指定数量的测试
`--maxWorkers=\|` | 设定运行测试的最大工作池数目
`--noStackTrace` | 禁止栈跟踪信息在测试结果输出中
`--notify` | 激活测试结果通知
`--onlyChanged` `-o` | 尝试确定根据当前存储库中哪些已经更改的文件来运行的测试
`--outputFile=` | 通过 `——json` 参数可以将测试结果写入到指定文件
`--passWithNoTests` | 允许在没有找到文件的情况下通过测试
`--projects  ... ` | 从指定路径中找到的一个或多个项目运行测试；也采用路径 `globs`
...
`--selectProjects  ... ` | 运行指定的测试项目
`--setupFilesAfterEnv  ... ` | 运行某些代码以在每次测试之前配置或设置测试框架的模块的路径列表
`--shard` | 测试套件分片以 `(?\d+)/(?\d+)` 的格式执行
`--showConfig` | 输出 `Jest` 配置，然后退出
`--silent` | 阻止所有测试通过控制台输出信息
`--testEnvironmentOptions=` | 带有将传递给 `testEnvironment` 的选项的 `JSON` 字符串
`--testLocationInResults` | 向测试结果添加 `location` 字段
`--testMatch glob1 ... globN` | `Jest` 用于检测测试文件的 `glob` 模式
`--testNamePattern=`, `-t` | 仅运行名称与正则表达式匹配的测试
`--testPathIgnorePatterns=\|[array]` | 在执行测试之前针对所有测试路径进行测试的单个或一组正则表达式模式字符串
`--testPathPattern=` | 在运行测试前，匹配的 `regexp` 模式字符串的测试文件路径
`--testRunner=` | 允许你指定自定义测试运行程序
`--testSequencer=` | 允许您指定自定义测试定序器
