...
```sh
mc admin logs [--last ] [--type ] TARGET [NODE]
```
...
参数 | 说明
---- | ----
`--access-key ` | 指定 Access Key（不指定则自动生成）
`--secret-key ` | 指定 Secret Key（不指定则自动生成）
`--comment ` | 为该密钥添加备注
`--expiry ` | 设置过期时间/有效期（不指定则为永久有效）
`--policy ` | 直接传入策略 JSON（为该密钥绑定权限）
`--policy-file ` | 从文件加载策略 JSON
`--description ` | 描述信息
...
`--token-type ` | 仅撤销指定类型的 Token（例如 `web`、`api`）
