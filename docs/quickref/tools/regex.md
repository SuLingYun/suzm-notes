...
范例 | 说明
:-|-
`\w` | “单词”字符   _(字母、数字或下划线)_
`\d` | 数字
`\s` | 空格   _(空格、制表符、vtab、换行符)_
`\W, \D, or \S` | 不是单词、数字或空格
`[\D\S]` | 表示不是数字或空格，两者都匹配
`[^\d\s]` | 禁止数字和空格
...
`[aeiou]`     | 匹配任何元音
`[^aeiou]`    | 匹配一个非元音
`r[iau]ng`    | 匹配**ring**、w**rang**le、sp**rung**等。
...
`(?>id\|identity)\b`   | 匹配 **id**，但不匹配 **id**entity
...
`re.finditer` | 返回一个可迭代的匹配对象  _(每个匹配一个)_
...
:- | -
`REGEXP` | 字符串是否匹配正则表达式
`REGEXP_INSTR()` | 匹配正则表达式的子字符串的起始索引   _（注意：仅限 MySQL 8.0+）_
`REGEXP_LIKE()` | 字符串是否匹配正则表达式   _(注意：仅 MySQL 8.0+)_
`REGEXP_REPLACE()` | 替换匹配正则表达式的子字符串   _（注意：仅限 MySQL 8.0+）_
`REGEXP_SUBSTR()` | 返回匹配正则表达式的子字符串   _(注意：仅 MySQL 8.0+)_
