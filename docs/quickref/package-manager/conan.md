...
```bash
$ conan remote add my_remote 
```

提供远程凭据

```bash
$ conan user -p  -r my_remote 
```
...
```bash
$ conan lock create /conanfile.py --user= --channel=
```

在 `conan create` 或 `conan install` 期间使用 lockfile：

```bash
$ conan  --lockfile conan.lock
```
