...
| `--worktree-base ` | 指定 worktree 基于的分支/引用         |
...
| `--yolo`                  | `--force` 的别名                        |
| `--api-key `         | 认证密钥（或 `CURSOR_API_KEY`）         |
| `-H, --header `        | 自定义请求头（`Name: Value`）           |
...
| `agent mcp list-tools `         | 列出指定 MCP 服务器的工具               |
| `agent mcp enable `             | 将 MCP 服务器加入批准列表               |
| `agent mcp disable `            | 禁用 MCP 服务器                         |
| `agent mcp login `              | 对 MCP 服务器进行认证                   |
...
Worktree 会在 `~/.cursor/worktrees//` 创建隔离的 git 分支环境，从而保持主工作区干净。
