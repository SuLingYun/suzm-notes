...
Command | Description
:-|:-
`screen -S ` | 使用会话名称开始新的屏幕会话
`screen -ls`       | 列出正在运行的会话/屏幕
`screen -x`        | 附加到正在运行的会话
`screen -r ` | 使用名称附加到正在运行的会话
`screen -dRR`      | “终极附加”
...
`Ctrl-A` `N` or `Ctrl-A` ``     | 切换到列表中的下一个窗口
`Ctrl-A` `P` or `Ctrl-A` `` | 切换到列表中的上一个窗口
`Ctrl-A` `"`                           | 查看窗口列表
`Ctrl-A` `W`                           | 显示窗口栏
...
:-|:-
`Ctrl-A` `D`     | 分离
`Ctrl-A` `D` `D` | 分离和注销   _(快速退出)_
`Ctrl-A` `:`     | 退出所有会话
`Ctrl-A` `C-\`   | 强制退出屏幕  _(不推荐)_
