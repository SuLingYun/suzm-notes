...
`dotnet new` 使用模板引擎创建项目或其他文件。可通过 `dotnet new search ` 查找模板。
...
`nuget add source` | 添加源 | `dotnet nuget add source  -n Private`
`nuget update source` | 更新源 | `dotnet nuget update source Private -s `
