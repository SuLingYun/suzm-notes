...
`OFS`          | `O`utput `F`ield `S`eparator  输出字段分隔符 _(default " ")_
`FS`           | input `F`ield `S`eparator   输入字段分隔符 _(default " ")_
`ORS`          | `O`utput `R`ecord `S`eparator   输出记录分隔符 _(default "\n")_
`RS`           | input `R`ecord `S`eparator   输入记录分隔符 _(default "\n")_
`FILENAME`     | 文件名
