...
`adb wait-for-device ` | 等到设备连接后再执行命令
`adb -s  ` | 指定某一台设备执行命令
`adb -d ` | 指定唯一 USB 设备执行命令
`adb -e ` | 指定唯一模拟器执行命令
...
`adb uninstall ` | 卸载应用
`adb uninstall -k ` | 卸载应用但保留数据和缓存目录
...
`adb shell pm path ` | 查看 APK 文件路径
`adb shell dumpsys package ` | 查看包的详细信息
`adb shell am force-stop ` | 强制停止应用
`adb shell monkey -p  1` | 启动指定包的默认入口
...
:-- | --
`adb push  ` | 从电脑复制文件到设备
`adb pull  [local]` | 从设备复制文件到电脑
`adb sync` | 同步构建输出到设备，常用于平台开发
`adb shell ls ` | 列出设备目录
`adb shell rm ` | 删除设备文件
`adb shell mkdir -p ` | 创建设备目录
...
`adb shell ` | 在设备上执行单条命令
`adb exec-out ` | 执行命令并将原始输出写到本机 stdout
`adb shell getprop` | 查看系统属性
`adb shell getprop ` | 查看指定系统属性
...
:-- | --
:-- | --
`adb shell input tap  ` | 点击屏幕坐标
`adb shell input swipe x1 y1 x2 y2` | 滑动屏幕
`adb shell input text ` | 输入文本，空格常写作 `%s`
...
:-- | --
:-- | --
`adb shell am start -n /` | 启动指定 Activity
`adb shell am start -a android.intent.action.VIEW -d ` | 打开 URL
`adb shell am start -a android.intent.action.VIEW -t image/*` | 打开图片查看器
`adb shell am broadcast -a ` | 发送广播
`adb shell am startservice -n ` | 启动服务
`adb shell am force-stop ` | 强制停止应用
`adb shell pidof ` | 查看进程 PID
...
`adb shell dumpsys ` | 输出指定服务信息
`adb shell dumpsys  -h` | 查看指定服务的帮助
`adb shell dumpsys battery` | 查看电池状态
`adb shell dumpsys meminfo ` | 查看应用内存信息
...
:-- | --
:-- | --
`adb pair :<pairing-port>` | 使用设备上显示的配对端口进行配对
`adb connect :<debug-port>` | 连接已配对设备的调试端口
`adb disconnect :<debug-port>` | 断开指定无线设备
