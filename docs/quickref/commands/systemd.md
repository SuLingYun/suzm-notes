...
:- | -
:- | -
`systemctl stop service` | **停止**正在运行的服务
...
`chkconfig –list` | `systemctl list-unit-files –type=service` (or)   `ls /etc/systemd/system/*.wants/` | 打印一个服务表，列出每个配置的运行级别打开或关闭
...
SysVinit | Systemd | 说明
:- | - | -
`telinit 3` | `systemctl isolate multi-user.target`   OR `systemctl isolate runlevel3.target`  OR `telinit 3` | 更改为多用户运行级别
