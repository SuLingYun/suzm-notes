...
| yum              | RedHat和CentOS的软件包安装和管理                 | 安装  `ansible all -m yum -a "name=httpd state=present"`  `ansible all -m yum -a "name=httpd state=installed"`  卸载  `ansible all -m yum -a "name=httpd state=absent"`  `ansible all -m yum -a "name=httpd state=removed"` |
| `service`和`systemd` | 用于管理远程主机的服务                           | `ansible all -m systemd -a "name=httpd state=started enabled=yes"`  `ansible all -m systemd -a "name=httpd state=restarted"` |
