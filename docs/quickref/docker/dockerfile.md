...
指令定义了一个变量，在构建时通过 `docker build` 命令使用 --build-arg `=` 标志将其传递给构建器。
...
`temp?` | 排除根目录中名称为  `temp` 的单字符扩展名的文件和目录
...
`COPY [--chown=:] ... ` | 将上下文中的路径复制到位置 `dest` 的容器中
`ADD [--chown=:] ... ` | 与 `COPY` 相同，但解压缩存档并接受 http url。
`RUN ` | 在容器内运行任意命令。
`USER [:]` | 设置默认用户名。
